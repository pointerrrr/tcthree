...
type CSharpAlgebra clas memb stat expr
    = (  Token -> [memb] -> clas
      ...
      ,  ( Decl                                 -> stat
         ...
         , expr                                 -> stat
         , [stat]                               -> stat
         , stat -> expr -> expr -> expr -> stat -> stat   -- for loop support (assignment 5)
         )

      ,  ( Token                 -> expr
         , Token                 -> expr
         , Token -> expr -> expr -> expr
         , Token -> [expr]       -> expr -- method call (assingment 6)
         )
      )
...
foldCSharp (c1, (m1,m2), (s1,s2,s3,s4,s5,s6,s7), (e1,e2,e3,e4)) = fClas
  where
    ...
    fStat (StatFor    d e1 e2 e3 s)= s7 (fStat d) (fExpr e1) (fExpr e2) (fExpr e3) (fStat s)  -- assignment 5
    fExpr (ExprConst  con)         = e1 con
    fExpr (ExprVar    var)         = e2 var
    fExpr (ExprOper   op e1 e2)    = e3 op (fExpr e1) (fExpr e2)
    fExpr (ExprCall   m es)        = e4 m (map fExpr es)
